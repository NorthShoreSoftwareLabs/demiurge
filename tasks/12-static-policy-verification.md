...
Tracking: unassigned. Design notes from a brainstorm session, not yet an epic.
...
This spec covers one reusable mechanism for verifying declared policy against
...
- Declarations hoist through the module graph. The build computes the effective
  ...
  for it. `docs/05-platform-features.md` already asks audit output to explain
...
- RSC is the answer, and the repo already chose it: `docs/03-implementation-roadmap.md`
  Phase 6, plus four settled decisions in `tasks/06-rendering-ssr-streaming-rsc.md`
  covering Flight under strict CSP.
- Islands should be deleted from `tasks/06` and `docs/07-feature-inventory.md`.
  It appears once, in a feature list, with no decisions and no example required.
  Leaving two designed answers to one settled question is how a roadmap rots.
