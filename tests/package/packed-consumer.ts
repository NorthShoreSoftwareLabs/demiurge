...
try {
  ...
  writeFileSync(
    ...
    [
      ...
      `import { createVercelFunction, vercelNode, vercelNodeAdapter } from "@demiurgejs/core/vercel";`,
      ...
      `import { assertDocument, assertSecurity, createApplicationTest, createTestClock, createTestRequest } from "@demiurgejs/core/testing";`,
      `import { createStaticOutputTest, inspectStaticOutput, readStaticOutputManifest, verifyStaticOutput, verifyStaticProvider } from "@demiurgejs/core/static/testing";`,
      ...
      `for (const [name, value] of Object.entries({ createApplicationTest, createEdgeAssetHandler, createEdgeRequestHandler, createEncryptedCookieSession, createKvCacheStore, createKvSessionStore, createMemorySessionStore, createMutationAction, createNodeServer, createRedisCacheStore, createRedisSessionStore, createRequestHandler, createSessionManager, createSignedCookieSession, createStaticOutputTest, createTestClock, createTestRequest, createVercelFunction, defineConfig, demiurge, generateStaticOutput, hydrateFileRouter, inspectStaticOutput, mutation, MutationSubmit, MutationValidationError, page, readStaticOutputManifest, unstable_createRouteManifest, useMutationAction, vercelNode, verifyAdapterContract, verifyCacheStoreContract, verifyDeploymentContract, verifySessionStoreContract, verifyStaticOutput, verifyStaticProvider })) {`,
      ...
      `}`,
      `const packedApplication = createApplicationTest(async (request) => new Response(request.url));`,
      `const packedApplicationResponse = await packedApplication.request("/application-test");`,
      `if (await packedApplicationResponse.text() !== "https://demiurge.test/application-test") {`,
      `  throw new Error("Expected the packed application test boundary.");`,
      `}`,
      `const packedClock = createTestClock(1_000);`,
      `if (packedClock.advance(1) !== 1_001 || packedClock.now() !== 1_001) {`,
      `  throw new Error("Expected the packed deterministic test clock.");`,
      `}`,
      `const packedRequest = createTestRequest("/request-fixture", { method: "POST" });`,
      `if (packedRequest.url !== "https://demiurge.test/request-fixture" || packedRequest.method !== "POST") {`,
      `  throw new Error("Expected the packed standard request fixture.");`,
      `}`,
      `const packedDocument = new Response('<title>Packed</title><script data-demiurge-document-contribution src="/assets/packed.js" nonce="packed-nonce"></script>', { headers: { "cache-control": "private, no-store", "content-security-policy": "script-src 'nonce-packed-nonce'" } });`,
      `await assertDocument(packedDocument, { scripts: [{ src: "/assets/packed.js" }], title: "Packed" });`,
      `await assertSecurity(packedDocument, { nonce: true });`,
      ...
      `}`,
      `if (vercelNodeAdapter.name !== "vercel-node" || !vercelNodeAdapter.capabilities.streaming || vercelNode().runtime !== "nodejs22.x") {`,
      `  throw new Error("Expected the packed Vercel Node adapter contract.");`,
      `}`,
      ...
      `}`,
      `if (packedKvStore.capabilities.atomicity !== "best-effort") {`,
      `  throw new Error("Expected the packed KV cache store to declare best-effort atomicity.");`,
      `}`,
