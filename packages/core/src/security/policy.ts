...
function applyPolicyNeed(
  ...
  // SAFETY: the directive value is a CSP source list or a replacement. The cast labels it with the directive value union.
...
function mergeCsp(
  ...
  for (const [name, value] of cspDirectiveEntries(override)) {
    // SAFETY: the indexed base value is a CSP directive value or undefined. The cast labels it with the directive value union.
...
function setCspDirective(
  ...
  if (name === "reportUri") {
    // SAFETY: the reportUri directive value is an array of reporting endpoint URL strings. The cast labels it with that specific array type.
...
function cspDirectiveEntries(policy: ContentSecurityPolicy) {
  // SAFETY: Object.entries returns the policy entries as string keyed pairs. The cast narrows the keys to the policy key type.
