...
export function limitRequestBody(
  ...
  // TYPE-EVIDENCE: the duplex field is a runtime extension required for stream request bodies. The cast adds it to the standard init type.
...
export function parseBodySize(value: number | string) {
  ...
  // TYPE-EVIDENCE: the regex restricts the unit to a byte size suffix. The lowercase value is therefore a key of the byte units map.
