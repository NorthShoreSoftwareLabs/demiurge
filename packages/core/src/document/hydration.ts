...
export function readInitialRouteData(source: Document) {
  ...
  // SAFETY: the tagName check confirms the element is a template element. The cast adds the template content property.
  ...
  try {
    // SAFETY: the serialized value came from serializeInitialRouteData which emits an initial route data shape. The cast restores that shape after parsing.
