...
void test("builds the version from the unreleased package version", () => {
  assert.equal(
...
void test("moves to the next patch when the package version is already released", () => {
...
void test("refuses a package version that is not a semantic version", () => {
  assert.throws(() => nightlyBaseVersion("latest"), /not a semantic version/);
});

void test("refuses a commit that is not a Git object name", () => {
  assert.throws(
...
void test("reads the source commit back out of a published nightly version", () => {
...
void test("publishes when the nightly channel is empty", () => {
...
void test("skips when main has not advanced past the published nightly", () => {
...
void test("publishes when main advanced past the published nightly", () => {
  const plan = planNightlyRelease({
    commit,
    date,
    packageVersion: "0.2.0-beta.2",
...
void test("refuses a published nightly version it cannot read a commit from", () => {
  assert.throws(
    () =>
      planNightlyRelease({
        commit,
...
void test("always publishes under the nightly dist-tag", () => {
  assert.deepEqual(
...
void test("refuses to publish under the latest dist-tag", () => {
...
void test("refuses to publish a stable or deliberate prerelease version", () => {
...
void test("adds the dry-run argument on request", () => {
...
void test("recognizes only a complete nightly version", () => {
...
void test("writes only a nightly version into the manifest", () => {
